...
function App() {
  ...
  return (
    <div className="app">
      <Navbar user={profile} onNavigate={setView} onLogout={() => { supabase.auth.signOut(); setUser(null); setProfile(null); setView('landing'); }} />
      
      {view === 'landing' && <LandingView onNavigate={setView} />}
      {view === 'register' && <RegisterView supabase={supabase} onSuccess={(u, p) => { setUser(u); setProfile(p); setView('dashboard'); }} showToast={showToast} />}
      {view === 'login' && <LoginView supabase={supabase} onSuccess={(u, p) => { setUser(u); setProfile(p); setView('dashboard'); }} showToast={showToast} />}
      {view === 'iub' && <IUBView user={user} supabase={supabase} showToast={showToast} onNavigate={setView} />}
...
function RegisterView({ supabase, onSuccess, showToast }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      onSuccess(authData.user, { ...form, id: authData.user.id });
    } catch (error) {
...
function IUBView({ user, supabase, showToast, onNavigate }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const codigoIub = `TM-${prefix}-${form.segmento || 'LOC'}-${form.ciudad.substring(0,3).toUpperCase()}-${form.zona.substring(0,3).toUpperCase()}-${Math.floor(form.area/10)}-${Math.floor(form.presupuesto/1000)}-${Math.floor(Math.random()*10000).toString().padStart(4,'0')}`;
  ...
  return (
    <div style={{ padding: '40px 24px', maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ background: 'linear-gradient(135deg, #E74C3C 0%, #C0392B 100%)', color: 'white', padding: '40px', borderRadius: '20px', marginBottom: '32px' }}>
        <h2 style={{ color: 'white', marginBottom: '8px' }}>Hola, {profile?.nombre} </h2>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
        <div style={{ background: 'white', padding: '28px', borderRadius: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
          ...
          {iubs.length === 0 ? (
            <p style={{ color: '#7F8C8D', textAlign: 'center', padding: '20px' }}>Aún no tienes IUBs.</p>
          ) : (
            iubs.map(iub => (
        ...
        <div style={{ background: 'white', padding: '28px', borderRadius: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
          ...
          {matches.length === 0 ? (
            <p style={{ color: '#7F8C8D', textAlign: 'center', padding: '20px' }}>Configura un IUB para recibir matches.</p>
          ) : (
            matches.map(match => (
